...
resource "azuread_application_owner" "ad_owner" {
  depends_on = [azuread_application.azapp01, azuread_service_principal.azadsp01]
  application_object_id = azuread_application.azapp01.object_id
  owner_object_id = data.azuread_client_config.azclconfig01.object_id
